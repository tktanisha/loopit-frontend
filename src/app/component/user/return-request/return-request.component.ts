...
@Component({
  ...
  imports: [LoaderComponent, ReturnStatusPipe, CommonModule, Toast, TableModule, ButtonModule],
...
export class ReturnRequestComponent {
  ...
  getAllRequest() {
    this.isLoading = true;
    this.allRequestSubject = this.returnRequestService.GetAllReturnRequests().subscribe({
      next: (res: any) => {
        this.allReturnRequests = res.data ?? [];
      ...
      .subscribe({
        next: (res: any) => {
          ...
          this.messageService.add({
            severity: 'success',
            summary: 'Success',
            detail: 'Successfully Accepted request  ',
            life: 3000,
          });
          this.getAllRequest();
        },
        error: err => {
          ...
          this.messageService.add({
            severity: 'error',
            summary: 'Error',
            detail: 'Failed to update requests.',
          });
        },
      });
  }
