// BEGIN AI-generated - Following QuizApplication testing pattern

import { ComponentFixture, TestBed, fakeAsync, tick } from '@angular/core/testing';
import { NO_ERRORS_SCHEMA } from '@angular/core';
import { of, throwError, Subscription } from 'rxjs';
import { MessageService } from 'primeng/api';

import { GetAllBuyRequestComponent } from './buy-request.component';
import { BuyRequestService } from '../../../service/buy-request.service';
import { AuthService } from '../../../service/auth.service';
import { BuyRequestResponse } from '../../../models/buy-request';
import { LoggedInUser } from '../../../models/logged-in-user';

// We test GetAllBuyRequestComponent because it handles:
// - Fetching all buy requests
// - Filtering buy requests by user

describe('GetAllBuyRequestComponent', () => {
  let component: GetAllBuyRequestComponent;
  let fixture: ComponentFixture<GetAllBuyRequestComponent>;
  let mockBuyRequestService: jasmine.SpyObj<BuyRequestService>;
  let mockAuthService: jasmine.SpyObj<AuthService>;
  let mockMessageService: jasmine.SpyObj<MessageService>;

  const mockUser: LoggedInUser = {
    name: 'Test User',
    user_id: 'user-1',
    role: 'user',
    exp: new Date(),
    tokenExpirationDate: new Date()
  } as unknown as LoggedInUser;

  const mockBuyRequests: BuyRequestResponse[] = [
    { buy_request: { requested_by: 'user-1' }, product: {} } as unknown as BuyRequestResponse,
    { buy_request: { requested_by: 'user-2' }, product: {} } as unknown as BuyRequestResponse
  ];

  beforeEach(async () => {
    // Create spy objects for dependencies
    mockBuyRequestService = jasmine.createSpyObj('BuyRequestService', ['GetAllRequest']);
    mockAuthService = jasmine.createSpyObj('AuthService', ['getUser']);
    mockMessageService = jasmine.createSpyObj('MessageService', ['add']);

    // Setup default return values
    mockBuyRequestService.GetAllRequest.and.returnValue(of(mockBuyRequests));
    mockAuthService.getUser.and.returnValue(mockUser);

    await TestBed.configureTestingModule({
      imports: [GetAllBuyRequestComponent],
      providers: [
        { provide: BuyRequestService, useValue: mockBuyRequestService },
        { provide: AuthService, useValue: mockAuthService },
        { provide: MessageService, useValue: mockMessageService }
      ],
      schemas: [NO_ERRORS_SCHEMA]
    }).compileComponents();

    fixture = TestBed.createComponent(GetAllBuyRequestComponent);
    component = fixture.componentInstance;
    
    // Initialize requestSubject with a dummy subscription to prevent ngOnDestroy errors
    component.requestSubject = new Subscription();
  });

  afterEach(() => {
    // Safe cleanup
    if (component.requestSubject) {
      component.requestSubject.unsubscribe();
    }
  });

  // --- Basic instantiation ---

  it('should create the buy-request component', () => {
    // Assert
    expect(component).toBeTruthy();
  });

  it('should initialize with isLoading as false', () => {
    // Assert
    expect(component.isLoading).toBeFalse();
  });

  it('should initialize with empty allBuyRequests array', () => {
    // Assert
    expect(component.allBuyRequests).toEqual([]);
  });

  it('should initialize with empty BuyRequestOfUser array', () => {
    // Assert
    expect(component.BuyRequestOfUser).toEqual([]);
  });

  // --- ngOnInit ---

  it('should get logged in user on init', fakeAsync(() => {
    // Act
    component.ngOnInit();
    tick();

    // Assert
    expect(mockAuthService.getUser).toHaveBeenCalled();
    expect(component.loggedInUser).toEqual(mockUser);
  }));

  it('should fetch all buy requests on init', fakeAsync(() => {
    // Act
    component.ngOnInit();
    tick();

    // Assert
    expect(mockBuyRequestService.GetAllRequest).toHaveBeenCalled();
    expect(component.allBuyRequests).toEqual(mockBuyRequests);
  }));

  it('should filter buy requests by user', fakeAsync(() => {
    // Act
    component.ngOnInit();
    tick();

    // Assert
    expect(component.BuyRequestOfUser.length).toBe(1);
  }));

  it('should show error message when fetch fails', fakeAsync(() => {
    // Arrange
    mockBuyRequestService.GetAllRequest.and.returnValue(throwError(() => new Error('Network error')));

    // Act
    component.ngOnInit();
    tick();

    // Assert
    expect(component.isLoading).toBeFalse();
    expect(mockMessageService.add).toHaveBeenCalledWith(jasmine.objectContaining({
      severity: 'error',
      summary: 'Error'
    }));
  }));
});

// END AI-generated - Following QuizApplication testing pattern
